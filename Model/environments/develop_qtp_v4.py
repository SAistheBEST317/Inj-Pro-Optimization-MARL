...
from environments.env_utils_qtp import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
class development_qtp4(Env):  
    #path 为不带后缀的母文件名
...
#计算多部模拟的效益
    ...
    # agent 看到的饱和度和压力场
        ...
        self.total_well_loc = np.concatenate([self.old_well_loc, self.well_loc], axis = 0)#n,2
    ...
    def step(self, action):# [n] #
        ...
        run_path = write_and_run(self.args, self.path, self.well_list, action, self.comp_grid, self.step_num, self.step_time, self.args.init_step, self.record)
        if self.step_num == 0: 
            restart = False
    ...
    def reset(self, seed=None,options={'ith': 0, 'record' : None}):#reset 就是运算第0步, add_well [n,2]
        ...
        self.well_loc = np.stack([np.random.randint(low,high) for i in range(self.args.n_new_well)],axis=0) #[n_new, 2] 注意是y,x
        self.comp_grid = self.perf_2d_2_3d( self.well_loc, perf_start=0, perf_end = 6)
#重置文件
