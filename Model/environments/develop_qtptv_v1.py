...
#1!!!!注意，这个程序里面
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(4, self.args.grid_dim[0], self.args.grid_dim[1], self.args.grid_dim[2]), dtype=np.float)
            
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(self.args.action_dim,), dtype=np.float)
        
        assert (self.args.state_dim[0] == self.observation_space.shape[0] and self.args.state_dim[1] == self.observation_space.shape[1]), 'args 设定的state dim 与env的ob dim不一致'
