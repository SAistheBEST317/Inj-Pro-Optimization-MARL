...
def write_and_run (args, path, well_list, quantities, comp_grid, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
    ...
    if step_num == 0:#
        ...
        if quantities is None:
            quantities = np.array([-50 for i in range(args.n_old_well-1)] + [50] + [50 for i in range(comp_grid.shape[0])]).astype(int)
        else:
        ...
        assert len(well_list_total) <= args.well_num, f'井列表数量超出,总井列表{well_list_total},{comp_grid.shape[0]}'
