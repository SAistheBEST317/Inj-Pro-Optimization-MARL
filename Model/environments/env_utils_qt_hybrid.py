...
def write_and_run (args, path, well_list, well_type, quantities, comp_grid, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
    ...
    if step_num == 0:#
        ...
        if quantities is None:
            quantities = np.array([1 for i in range(len(well_list)+ comp_grid.shape[0])]).astype(int)
        else:
        ...
        for num, name in enumerate(inj_list):
            file.write(f'{name}   {type_dict[inj_medium_list[num]]}    OPEN    RESV    1*  {inj_value_list[num]}    600 /  \n')#[0]是为了去掉结果中的[]符号
        file.write('/ \n')
        file.write('\n')
        file.write('WCONPROD\n')
