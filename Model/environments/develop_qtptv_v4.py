...
from environments.env_utils_qtptv import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
class development_qtptv4(Env):  
    #path 为不带后缀的母文件名
...
#1!!!!注意，这个程序里面
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(3*args.well_num*self.args.grid_dim[0] * 2*args.well_range* 2*args.well_range,), dtype=np.float)
            
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(self.args.action_dim,), dtype=np.float)
        
        assert self.args.state_dim[0] == self.observation_space.shape[0] , f'args 设定的state dim{self.args.state_dim[0]} 与env的ob dim{self.observation_space.shape[0]}不一致'# self.well_type = spaces.Box(#注采量范围）
        #         low = 0., 
        #         high = 1, 
...
#计算多部模拟的效益
    ...
    # agent 看到的饱和度和压力场
        ...
        well_step = np.ones(well_sat.shape) * self.step_num
        state = np.stack([well_sat, well_pres, well_step],axis = 1).flatten()#[n,2,z,r,r]
    ...
    def step(self, action):# [n] #len = n_agent(number of wells), each(2,)(control,type)  
        #因为每一步他要先都step之前的ob所以，step的升级必须再当前步
        self.step_num += 1

        assert self.args.well_style =='vertical',' 本程序只针对直井'
        assert self.args.well_num == (self.args.n_old_well + self.args.n_new_well)
        assert self.args.action_dim == 2 + self.args.well_num,'action 维度不对'
        split = 2
        #还原尺度
        low=np.array([1,1])
        high=np.array([self.args.grid_dim[-2],self.args.grid_dim[-1]])
        position = ((action[:split] * (high-low)/2) + (low+high)/2).astype(int)
        # print('000000000000000000000000', position)
        if self.well_posi.shape[0]< (self.args.n_new_well + self.args.n_old_well):
            # self.new_well_near = 1/abs((position.reshape(1,split)[:,:2] - self.well_posi[:,:2]).sum(axis = -1).min()+0.0001)
            self.well_posi = np.concatenate([self.well_posi, position.reshape(1,-1)],axis = 0)#先换成list添加一个2长度的array，在转化回到2维度的数组 最后维度[n_now,2] 这个n_now 表示现有的新井数量
            self.step_add_well = 1
        else:
            self.well_posi = self.well_posi #到了上限就不再增加井了
            self.step_add_well = 0
            
        if self.well_style =='vertical':
            self.comp_grid = self.perf_2d_2_3d(self.well_posi, perf_start=1, perf_end = 6)#[n,2]==>[n,g,3]
        elif self.well_style =='horizon':
            self.comp_grid = self.perf_grid(self.well_posi, grid_dim = self.args.grid_dim, delt_grid = self.args.grid_delta) #[n,5] ==> [n,grid,3] 这个3是grid坐标 坐标是[x,y,z]
        quantities = action[split:split + self.args.well_num]#保证quantities数量与已有的井匹配
        # print('????????????????????????????',self.comp_grid, quantities,self.well_posi)
        
        
        run_path = write_and_run(self.args, self.path, self.well_list, quantities, self.comp_grid,  self.step_num, self.step_time, self.args.init_step, self.record)
        # if self.step_num == 0: 
        #     restart = False
        ...
        self.indice = check_indice(run_path + '.RSM', time_len=1, key_words = self.key_list_total,restart = False)
        ...
        return next_state, reward , truncated, done, {'real_r':profit,'ep_steps': self.step_num} #必须要保证4个或者5个结果的输出（这个要求，不然会报错）
