...
from environments.env_utils_p import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
class development_p1(Env):  
    #path 为不带后缀的母文件名
...
#1!!!!注意，这个程序里面
        ...
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(2,), dtype=np.float)
        ...
        assert self.args.action_dim == self.action_space.shape[0],'action 维度不对'
        assert self.args.n_new_well == self.args.max_steps, '由于每一步添加新井，要求新井数==最大步数，目前新井{self.args.n_new_well}，最大步{self.args.max_steps}'
...
#计算多部模拟的效益
    ...
    #标准化观测结果
        ...
        # plt.show()
        state = np.stack([sat_ob, pres_ob, step_map],axis = 0)#[1,4,z,r,r]
    ...
    def step(self, action):# [2] # 每一次加入一口井
        #因为每一步他要先都step之前的ob所以，step的升级必须再当前步
        self.step_num += 1
        assert action.shape[0] == 2 ,' 井action维度不对 {action.shape}'
        #转化尺度
        low = np.ones(2)
        high = np.array(self.args.grid_dim[-2:])
        action = self.inv_norm(action, low, high)
        
        if self.well_loc is None:
            self.well_loc = action.reshape(1,2) 
        else:
            self.well_loc = np.concatenate([self.well_loc, action.reshape(1,2)], axis = 0)#[n+1,2]
        

        
        self.comp_grid = self.perf_2d_2_3d( self.well_loc, perf_start=0, perf_end = 6)#新井的射孔数据
        
        run_path = write_and_run(self.args, self.path, self.well_list, None,  self.comp_grid, self.step_num, self.step_time, self.args.init_step, self.record)
        if self.step_num == 0: 
            restart = False
...
#重置文件
        ...
        run_path = write_and_run(self.args, self.path, self.well_list, None, None, self.step_num, 1, self.args.init_step, self.record) 
