...
from environments.env_utils_her import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
import torch
class development_her_v4(Env):  
    #path 为不带后缀的母文件名
...
#1!!!!注意，这个程序里面
        ...
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(self.args.action_dim + self.args.state_dim + self.args.action_dim,), dtype=np.float) # action_high + action_low
        #非常高级的定义action space方法
        # self.action_space_high = spaces.Tuple([spaces.MultiDiscrete([self.args.well_type_num for i in range(self.args.well_num)]),
        #                                                            spaces.Box(low=-1, high=1, shape=(self.args.state_dim ,), dtype=np.float)]) #
        self.action_space_high = spaces.Box(
            low=-1, high=1, shape=(self.args.action_dim + self.args.state_dim,), dtype=np.float) #

        self.action_space_low = spaces.Box(
            low=-1, high=1, shape=(self.args.action_dim,), dtype=np.float) #

        assert (self.args.state_dim == self.observation_space.shape[0] ), 'args 设定的state dim 与env的ob dim不一致{self.args.state_dim}, {self.observation_space.shape[0]}'
...
#计算多部模拟的效益
        ...
        print('in reward check out indice, ','oil',well_indice.sum(0)[0],'w_inj',well_indice.sum(0)[2],'g_inj',well_indice.sum(0)[3])
    ...
    # agent 看到的饱和度和压力场
        #选定范围半径
        ob_range = self.args.well_range
        #场数据前后扩展10，用0充填
        padding_sat = np.zeros([sat_ob.shape[0],sat_ob.shape[1]+ob_range*2,sat_ob.shape[2]+ob_range*2])
        padding_sat[:,ob_range:-ob_range,ob_range:-ob_range] = sat_ob[:,]
        padding_pres = np.zeros([pres_ob.shape[0],pres_ob.shape[1]+ob_range*2,pres_ob.shape[2]+ob_range*2])
        padding_pres[:,ob_range:-ob_range,ob_range:-ob_range] = pres_ob
        # print('in develop check error',padding_sat.shape,ob_range)

        well_ind = self.well_loc - 1#变成序号#[n,2]
        #序号padding 
        well_ind_padding = well_ind + ob_range
        #生成单井sat
        well_sat_list = []
        for i in range(well_ind.shape[0]):
            w_sat = padding_sat[:,well_ind_padding[i,0]-ob_range:well_ind_padding[i,0]+ob_range, well_ind_padding[i,1]-ob_range:well_ind_padding[i,1]+ob_range]#[z,r,r]
            well_sat_list.append(w_sat)        
        well_sat = np.stack(well_sat_list,axis = 0)#n,z,r,r
        
        #生成单井pres
        well_pres_list = []
        for i in range(well_ind.shape[0]):
            w_pres = padding_pres[:,well_ind_padding[i,0]-ob_range:well_ind_padding[i,0]+ob_range, well_ind_padding[i,1]-ob_range:well_ind_padding[i,1]+ob_range]#[z,r,r]
            well_pres_list.append(w_pres)        
        well_pres = np.stack(well_pres_list,axis = 0)#n,z,r,r

        # print('in develop check output dim',well_sat.shape, well_pres.shape)
        step_ob = self.norm((self.step_num-self.args.init_step), 0 , self.args.max_steps)
        well_step = np.ones(well_sat.shape) * step_ob
        state = np.stack([well_sat, well_pres, well_step],axis = 1).mean(-1).mean(-1).mean(-1).flatten()#[n*3]

        assert state.shape[0] == self.args.state_dim, f'输出的state维度和设定不一致，输出{state.shape[0]},设定的是{self.args.state_dim}'
        
        # print('in develop env check state------------------', state.shape)
        return state #n,2,z,r,r
        
    def step(self, action):# [2n] #[well type, quantities]
        #因为每一步他要先都step之前的ob所以，step的升级必须再当前步
        # print('in develop check step action-----------', action)
        self.step_num += 1
        assert action.shape[0] == self.args.well_num*2 ,' action 维度不对{action.shape}'
        well_type, quantities = action[:self.args.action_dim], action[self.args.action_dim:]
        run_path = write_and_run(self.args, self.path, self.well_list, well_type, quantities, None, self.step_num, self.step_time, self.args.init_step, self.record)
        if self.step_num == 0: 
            restart = False
        ...
        #导出实际的action
        control_list = ['WLPR','WWIR','WGIR']
        ob_list = []
        for a in range(len(control_list)):
            if control_list[a] == 'WGIR':
                divide = self.args.rs
            else:
                divide = 1
            data = self.indice[self.key_list_total.index(control_list[a])].transpose() / divide#[n,1]
            ob_list.append(data)
            # print('in develop check realaction shape??????????????????????????', data.shape)
        real_action = self.norm(np.concatenate(ob_list, axis = -1).sum(axis = -1), self.args.control_min, self.args.control_max) #[n]
        ...
        if done or truncated :
            print('environment done because','rewrd', reward,'at step:',self.step_num)
            rmtree( self.creat_folder_n, ignore_errors = True)
        
        # print('in develop step check state shape', next_state.shape)
        return next_state, reward , truncated, done, {'real_r':profit,'real_action': real_action,'achieve': next_state} #必须要保证4个或者5个结果的输出（这个要求，不然会报错）
    ...
    def reset(self, seed=None,options={'ith': 0, 'record' : None}):#reset 就是运算第0步, add_well [n,2]
        ...
        if seed is not None:
            ...
            seed = np.random.randint(1) #这样可以让每次初始化结果都不同
            torch.manual_seed(seed)
            np.random.seed(seed)
...
#重置文件
        ...
        # self.well_list = None
        self.well_loc = pd.read_excel(self.args.posi_path).values#[n,2] #第一列序号，第二第三列是y,x 坐标 注意！！是y, x 坐标

        run_path = write_and_run(self.args, self.path, self.well_list, None,  None, None, self.step_num, 1, self.args.init_step, self.record) 
