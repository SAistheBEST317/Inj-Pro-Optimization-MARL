...
from environments.env_utils_qt_hybrid import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
class development_qt_hybrid_v1(Env):  
    #path 为不带后缀的母文件名
...
#1!!!!注意，这个程序里面
        ...
        assert self.args.control_min >= 0, 'hibrid模式下 control 只表示液量因此必须大于0'
...
#计算多部模拟的效益
    ...
    def step(self, action):# [2n] #
        assert len(action.shape) == 1, '环境里的action带有其他维度{action.shape}'
        #因为每一步他要先都step之前的ob所以，step的升级必须再当前步
        self.step_num += 1
        assert action.shape[0] == self.args.well_num *2,' action 维度不对，{action.shape[0]}, {self.args.well_num}'
        assert self.args.action_dim == self.args.well_num*2, '设定的action dim 要等于well num‘'
        well_type , quantities = action[:self.args.action_dim//2], action[self.args.action_dim//2:]
        
        run_path = write_and_run(self.args, self.path, self.well_list, well_type, quantities, self.comp_grid, self.step_num, self.step_time, self.args.init_step, self.record)
        if self.step_num == 0: 
            restart = False
...
#重置文件
        ...
        run_path = write_and_run(self.args, self.path, self.well_list, None, None, self.comp_grid, self.step_num, 1, self.args.init_step, self.record) 
