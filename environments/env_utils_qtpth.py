...
def write_and_run (args, path, well_list, well_quantities, comp_grid, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
    ...
    if step_num == 0:#
        ...
        if well_quantities is None:
            quantities = np.array([0 for i in range(comp_grid.shape[0])]).astype(int)
        else:
        ...
        well_list_total = well_list + new_well_list[:(comp_grid.shape[0]-len(well_list))]

        for wt in range(quantities.shape[0]):
        ...
        if comp_grid.shape[0]> len(well_list) : #改情况下为有效的comp_grid
            ...
            for num in range(len(well_list),comp_grid.shape[0]):
                file.write(f'{well_list_total[num]} 1 {comp_grid[num,0,0]} {comp_grid[num,0,1]} 1* OIL 9* STD / \n')
            file.write('/\n')
            file.write('COMPDAT\n')
            for num in range(len(well_list),comp_grid.shape[0]):
                for grid in range(comp_grid[num].shape[0]):
                    if (comp_grid[num][grid]==0).any(): 
                        ...
                        file.write(f'{well_list_total[num]} {comp_grid[num,grid,0]} {comp_grid[num,grid,1]} {comp_grid[num,grid,2]} {comp_grid[num,grid,2]}  OPEN 2* 0.2 1* 0 3* / \n')
            file.write('/\n')
        file.write('WCONINJE\n')
