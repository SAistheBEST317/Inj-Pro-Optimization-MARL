...
def write_and_run (args, path, well_list, quantities, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
    ...
    if step_num == 0:#
        ...
        assert args.control_min >=0, '只控制量的设置下，注采量必须都是正的'
        if quantities is None:
            quantities = np.array([1 for i in range(len(well_list))]).astype(int)
        else:
        ...
        file.write('WCONINJE\n')
        file.write(f'INJECT1   WATER    OPEN    RESV    1*  {quantities[0]}    600 /  \n')#[0]是为了去掉结果中的[]符号
        file.write(f'INJECT3   WATER    OPEN    RESV    1*  {quantities[1]}    600 /  \n')#[0]是为了去掉结果中的[]符号
        file.write('/ \n')
        file.write('\n')
        file.write('WCONPROD\n')
        file.write(f'PROD1 1* RESV 4* {quantities[2]} 300 / \n')
        file.write('/\n')
