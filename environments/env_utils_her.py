...
def write_and_run (args, path, well_list, well_type_continous, quantities, comp_grid, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
    ...
    if step_num == 0:#
        ...
        assert args.control_min ==0 , 'bybrid 的quantities 只能是大于等于0'
        if quantities is None:
            quantities = np.array([1 for i in range(len(well_list)+ comp_grid.shape[0])])
        else:
            quantities = quantities * ((args.control_max - args.control_min)/2) + ((args.control_max + args.control_min)/2)

        if well_type_continous is None:
            well_type = [0 for i in range(args.well_num)]
        else:
            
            well_type = np.round(well_type_continous * ((args.well_type_num-1)//2) + ((args.well_type_num-1)//2)).astype(int)
            # print('in env util check well type', well_type_continous, np.round(well_type_continous * ((args.well_type_num-1)//2) + ((args.well_type_num-1)//2)), well_type)
        pro_list = []
        pro_value_list=[]
        inj_list = []
        inj_medium_list =[]
        ...
        for wt in range(quantities.shape[0]):
            ...
            if well_type[wt]==0 :
                ...
                inj_medium_list.append(well_type[wt])
                inj_value_list.append(quantities[wt] )
            
            elif well_type[wt]==2 :
                inj_list.append(well_list_total[wt])
                inj_medium_list.append(well_type[wt])
                inj_value_list.append(quantities[wt] * args.rs )#注汽量要加上气油比
        
        type_dict = { 0: 'OIL',1:'WATER',2:'GAS'} 

        print('本次选择的控制量是',quantities)
        print('本次选择的井类型是',well_type)
        ...
        for num, name in enumerate(inj_list):
            file.write(f'{name}   {type_dict[inj_medium_list[num]]}    OPEN    RATE    {inj_value_list[num]}    1*    600 /  \n')#[0]是为了去掉结果中的[]符号
        file.write('/ \n')
        file.write('\n')
        file.write('WCONPROD\n')
        for num, name in enumerate(pro_list):
            file.write(f'{name} 1* LRAT 3* {pro_value_list[num]} 1* 300 / \n')
        file.write('/\n')
