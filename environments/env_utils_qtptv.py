...
def write_and_run (args, path, well_list, well_quantities, comp_grid, step_num, interval, init_step = None,record_file = None):#path =/egg/EGG_MODEL_ECL , [ pro/inj, type]
...
# if comp_grid is None:
    ...
    if step_num == 0:#
        ...
        if well_list == None:
            well_list = ['INJECT1','INJECT2','INJECT3','INJECT4','INJECT5','INJECT6','INJECT7','INJECT8', 'PROD1','PROD2','PROD3','PROD4' ]
        case_name = path.split('/')[-1]
        inj_list = []#所处位置
        pro_list = []
        inj_value = []
        ...
        if well_quantities is None:
            quantities = np.array([0 for i in range(args.well_num)]).astype(int)
        else:
            quantities = well_quantities * ((args.control_max - args.control_min)/2) + ((args.control_max + args.control_min)/2)
            quantities = quantities.astype(int)
        assert len(quantities) == args.well_num, f'井生产数据不足,现在数量是{len(quantities)}'
        # print('+++++++++++++++++++++++++++ check quantities', well_quantities,quantities)
        #正的采油，负的注水,小于10就关井
        type_dict = {0: 'SHUT', 1: 'OIL',2:'WATER',3:'GAS'} 
        
        #计算井类型
        well_type = np.ones(quantities.shape)
        # print('in env util check quantities shape', quantities.shape,well_type.shape, well_quantities, quantities)
        pro_list = []
        pro_value_list=[]
        inj_list = []
        inj_value_list=[]
    
        well_list_total = well_list + new_well_list[:(comp_grid.shape[0]-len(well_list))] #保持变化的经数量
        assert len(well_list_total) == comp_grid.shape[0], '井列表数量不足'
        ...
        # inj_list = ['INJECT1','INJECT2','INJECT3','INJECT4','INJ。ECT5','INJECT6','INJECT7','INJECT8']
        # pro_list = ['PROD1','PROD2','PROD3','PROD4' ]
        #射孔层位
        
        # well_perf_in = (well_perf).astype(int) #[n, n of layers)
        
        # well_posi_x = position % args.grid_dim[-1]    #[n,1]
        # well_posi_y = position // args.grid_dim[-1]
        # well_posi = np.ones((args.n_new_well, 2))
        # well_posi[:,0] = well_posi_x
        # well_posi[:,1] = well_posi_y
        # well_posi = well_posi.astype(int)
        # if well_style =='vertical':
        #     comp_grid = perf_2d_2_3d(position, perf_start=1, perf_end = 6)#[n,2]==>[n,g,3]
        # elif well_style =='horizon':
        #     comp_grid = perf_grid(position, grid_dim = args.grid_dim, delt_grid = args.grid_delta) #[n,5] ==> [n,grid,3] 这个3是grid坐标 坐标是[x,y,z]
        print('目前开井数', comp_grid.shape[0])#[n]
        print('本次选择的井位置是',comp_grid[:,0,:2])
        # print('本次选择的井射孔是',comp_grid)
        ...
        file.write('BASIC=2 / \n')
        file.write('\n')
        #总的来说，没有开的井，要求定义和射孔关键字都不能出现，控制就更不能出现了
        if comp_grid.shape[0]> len(well_list) : #改情况下为有效的comp_grid
            file.write('WELSPECS\n')
            for num in range(len(well_list),comp_grid.shape[0]):
                file.write(f'{well_list_total[num]} 1 {comp_grid[num,0,-1]} {comp_grid[num,0,-2]} 1* OIL 9* STD / \n')
            file.write('/\n')
            file.write('COMPDAT\n')
            for num in range(len(well_list),comp_grid.shape[0]):
                for grid in range(comp_grid[num].shape[0]):
                    if (comp_grid[num][grid]==0).any(): 
                        ...
                        file.write(f'{well_list_total[num]} {comp_grid[num,grid,-1]} {comp_grid[num,grid,-2]} {comp_grid[num,grid,-3]} {comp_grid[num,grid,-3]}  OPEN 2* 0.2 1* 0 3* / \n')
            file.write('/\n')
        file.write('WCONINJE\n')
