...
#1!!!!注意，这个程序里面
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(args.well_num * args.grid_dim[0] * 3,), dtype=np.float)
...
#计算多部模拟的效益
    ...
    #标准化观测结果
        ...
        # plt.show()
        field_state = np.stack([sat_ob, pres_ob, step_map],axis = 0)#[3,z,r,r]
        print('+++++++++in develop check field',field_state.shape)
        
        well_state_list = []
        for w in self.well_loc:
            well_state_list.append(field_state[:,:,w[0]-1,w[1]-1].flatten()) #从坐标变成序号
        state = np.concatenate(well_state_list)
