...
from environments.env_utils_qtpth import write_and_run,  check_field, check_indice, npv_fun , agent_npv, agg_adj

from shutil import copy, copytree, rmtree
import seaborn as sns
import pandas as pd
class development_qtpth1(Env):  
    #path 为不带后缀的母文件名
...
#1!!!!注意，这个程序里面
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(3, self.args.grid_dim[0], self.args.grid_dim[1], self.args.grid_dim[2]), dtype=np.float)
...
#计算多部模拟的效益
        ...
        profit = (well_indice[:,0] * self.args.oil_price * self.args.interval \
                       ...
                       + well_indice[:,2] * self.args.inj_w_price * self.args.interval \
                           + well_indice[:,3] * self.args.inj_g_price * self.args.interval)/10000000 #[n]
    ...
    #标准化观测结果
        ...
        step_map = np.ones((well_map.shape)) * (self.step_num / self.args.max_steps)
        # sns.heatmap(well_map[0,...])
        # plt.show()
        agent_state = np.stack([sat_ob, pres_ob, well_map,step_map],axis = 0)#[1,4,z,r,r]
        

        print('in develop env check state', agent_state.shape)
    ...
    def step(self, action):# [n] #len = n_agent(number of wells), each(2,)(control,type)  
        ...
        if self.args.well_style == 'vertical':
            split = 2  #list([2],[n_old_well + n_new_well])#每一步只能打一口井
        elif self.args.well_style == 'horizon':
            split = 5
            
        #还原尺度
        low=np.array([1,1,50,0,0])
        high=np.array([self.args.grid_dim[-1],self.args.grid_dim[-2],self.args.horizon_len_max,self.args.horizon_dev_max,self.args.horizon_azi_max])
        ...
        if self.well_posi.shape[0]< (self.args.n_new_well + self.args.n_old_well):
            self.new_well_near = 1/abs((position.reshape(1,split)[:,:2] - self.well_posi[:,:2]).sum(axis = -1).min())
        ...
        quantities = action[split:split + self.well_posi.shape[0]]#保证quantities数量与已有的井匹配
        ...
        if self.add_step_num is not None:
            self.step_num = self.args.init_step + self.add_step_num
        else:
            self.step_num += 1
        ...
        # done = True if (self.step_num - self.args.init_step)>= self.args.max_steps else False#g_reward < 0 #+ np.array([self.step_num >= self.max_episode_length for i in range(action.shape[0])]) #or partial_obs['profit'] <= 0
        ...
        return next_state, reward , done, {'real_r':profit} #必须要保证4个或者5个结果的输出（这个要求，不然会报错）

    # def _get_drawbridge_angle(self):
    #     return -min(max(self.current_step - self._drawbridge_start, 0.)*0.4, 90.0)
  
    def reset(self,  ith = 0, record = None):#reset 就是运算第0步, add_well [n,2]
        ...
        if record is not None:
            ...
            self.record = None
#重置文件
        print('in develop rest file number is',ith)
        self.creat_folder_n =  self.creat_folder + str(ith)
    ...
    def perf_2d_2_3d(self, position,perf_start=1, perf_end = 6):#[n,2], 射孔顶层，射孔低层
            ...
            for line in range(position.shape[0]):
                ...
                for j in range(z_num):
                    well_single.append([position[line,0],position[line,1],j+1])
